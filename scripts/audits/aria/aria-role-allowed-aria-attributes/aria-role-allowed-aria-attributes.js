...
// prettier-ignore
export const ariaRoleAllowedAriaAttributesList = {
  alert: [
...
export function hasAriaRoleAllowedAriaAttributes() {
  return Array.from(document.querySelectorAll("[role]"))
    .filter((element) => isElementVisible(element))
    .flatMap((element) => {
      const validAttributes =
        ariaRoleAllowedAriaAttributesList[element.getAttribute("role")] || [];
      return Array.from(element.attributes)
        .filter(
          (attr) => attr.name.startsWith("aria") && !validAttributes.includes(attr.name)
        )
        .map((attr) => ({
...
export async function hasAriaRoleAllowedAriaAttributesEval(auditResults) {
  // https://dequeuniversity.com/rules/axe/4.10/aria-allowed-attr
  const ariaRoleAllowedAriaAttributes = await inspectedWindowEval(`
    const ariaRoleAllowedAriaAttributesList = ${JSON.stringify(ariaRoleAllowedAriaAttributesList)};
    const getUniqueSelector = ${getUniqueSelector.toString()};
    const isElementVisible = ${isElementVisible.toString()};
    const hasAriaRoleAllowedAriaAttributes = ${hasAriaRoleAllowedAriaAttributes.toString()};

    return hasAriaRoleAllowedAriaAttributes();
  `);
