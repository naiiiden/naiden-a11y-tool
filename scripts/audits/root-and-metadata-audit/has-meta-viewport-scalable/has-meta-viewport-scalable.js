...
export async function hasMetaViewportUserScalableNoOrZeroEval(auditResults) {
  ...
  if (
    metaViewport &&
    metaViewport.hasContentAttr &&
    (metaViewport.hasContentAttr.includes("user-scalable=no") ||
      metaViewport.hasContentAttr.includes("user-scalable=0"))
  ) {
    auditResults.push({
      ...rootAndMetadataErrors[4],
