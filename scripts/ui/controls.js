...
export function uiControls(runAudit) {
  const auditCheckboxes = document.querySelectorAll(
  ...
  const openAuditCheckboxesDropdownBtn = document.querySelector(
    "#open-audit-checkboxes-dropdown-button",
  );
  ...
  document.addEventListener("focusin", (event) => {
    if (
      auditCheckboxesDropdown.classList.contains("open") &&
      !auditCheckboxesDropdown.contains(event.target) &&
      !openAuditCheckboxesDropdownBtn.contains(event.target)
    ) {
      auditCheckboxesDropdown.classList.remove("open");
  ...
  function updateSelectAllButton() {
    const allCheckboxesChecked = Array.from(auditCheckboxes).every(
  ...
  function updateRunButton() {
    const someCheckboxesChecked = Array.from(auditCheckboxes).some(
      (checkbox) => checkbox.checked,
    );
